...
const SupportOptions = () => {
  const supportChannels = [
    {
      ...
      description: "Nuestra inteligencia artificial resuelve el 98% de problemas en menos de 1 minuto",
      icon: (
    ...
    },
    {
      ...
      description: "Conéctate directamente con nuestros ingenieros cuánticos",
      icon: (
    ...
    },
    {
      ...
      description: "Un especialista aparece en tu hogar en menos de 30 minutos",
      icon: (
        <svg className="h-10 w-10 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const handleAction = (channel) => {
    if (channel === "WhatsApp Prioritario") {
      window.open('https://wa.me/573001234567', '_blank');
    }
    // Otras acciones se implementarían aquí
  };
  ...
  return (
    <div className="bg-gray-800 bg-opacity-60 backdrop-blur-lg rounded-xl p-8 shadow-xl border-2 border-cyan-500 border-opacity-20">
      <h3 className="text-2xl font-bold text-white mb-6">Canales de Soporte</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {supportChannels.map((channel, index) => (
          <div key={index} className="bg-gray-900 p-6 rounded-lg border-2 border-gray-700 hover:border-cyan-400 transition-all duration-300">
            ...
            <h4 className="text-xl font-semibold text-white mb-2">{channel.name}</h4>
            <p className="text-gray-300 mb-4">{channel.description}</p>
            <button
              onClick={() => handleAction(channel.name)}
              className={`w-full bg-gradient-to-r ${channel.color} text-white py-2 px-4 rounded-lg font-medium hover:shadow-lg hover:shadow-cyan-500/30 transition-all duration-300`}
            >
              {channel.action}
            </button>
          </div>
